  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color: { value: new THREE.Color("#60b3f7") },
      },
      vertexShader: `
        uniform float time;
...
export function AnomalousMatterHero({
  ...
  return (
    <section className="relative w-full h-full bg-[#0d1b35] text-white overflow-hidden">
      <Suspense fallback={<div className="w-full h-full bg-[#0d1b35]" />}>
        <GenerativeArtScene />
      </Suspense>

      {/* Gradient overlay — fades from dark blue at bottom to transparent at top */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#0d1b35] via-[#0d1b3599] to-transparent z-10" />

      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-16 text-center px-6">
        <div className="max-w-md">
          <p className="text-xs font-mono tracking-widest text-[#60b3f7] uppercase mb-4">
            {title}
          </p>
